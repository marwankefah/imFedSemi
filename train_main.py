...
def test(args, epoch, net=None, save_mode_path=None, val=False):
    ...
    for key in dict_user.keys():
        ...
        client_AUC.append(round(AUROC_avg, 6))
        Accus_avg = np.array(Accus).mean();
        client_Acc.append(round(Accus_avg, 6))
        Senss_avg = np.array(Senss).mean();
        client_Sen.append(round(Senss_avg, 6))
        Specs_avg = np.array(Specs).mean();
        client_Spe.append(round(Specs_avg, 6))
        Preci_avg = np.array(Preci).mean();
        client_Pre.append(round(Preci_avg, 6))
        F1_avg = np.array(F1).mean();
        client_F1.append(round(F1_avg, 6))
...
def prepare_data(args, supervised_user_id, unsupervised_user_id):
    ...
    trans_weak = transforms.Compose([transforms.Resize((224, 224)),
    ...
    trans = dataset.TransformFixMatch(trans_weak, trans_strong)
    ...
    client_train_data, client_priors_corr, client_Pi = dataset.load_data_normal(train_dataset,
                                                                                [dict_users_train[i] for i in
                                                                                 unsupervised_user_id], clientnum=10)
...
if __name__ == '__main__':
    ...
    for i in unsupervised_user_id:
        trainer_locals.append(
            UnsupervisedLocalUpdate(args, unsup_train_datasets[i], client_Pi[i], client_priors_corr[i]))
    ...
    for com_round in range(args.rounds):
        ...
        if com_round * args.local_ep <= WARMUP:
            ...
            if not flag_create:
                ...
                for i in unsupervised_user_id:
                    ...
                    optimizer = torch.optim.Adam(net_locals[i].parameters(), lr=args.base_lr, betas=(0.9, 0.999),
                                                 weight_decay=5e-4)
        ...
        # Evaluation and Test
        if com_round % EVAL == 0:
            client_AUC, client_Acc, client_Sen, client_Spe, client_F1, val_loss = test(args, com_round, net_glob, None,
                                                                                       True)
